...
ndarray의 astype 메서드를 사용해서 배열의 dtpye을 다른 형으로 변환 가능하다. 위 코드는 정수형인 sw_arr배열에 astype을 사용해서 부동소수점 형식의 sw_float_arr 배열을 새롭게 만든 것이다.
...
위 코드는 부동소수점 형식의 sw_arr 배열을 astype을 사용해서 정수형으로 변환한 것이다. sw_arr 배열의 값이 변한 것을 확인할 수 있는데, 만약 부동소수점수를 정수형 dtype으로 변환하면 소수점 아래 자리는 버려진다.
...
숫자 형식의 문자열을 담고 있는 배열이 있다면 astype을 사용하여 숫자로 변환할 수 있다. sw_numeric_strings 이름의 문자열 배열을 생성하고, astype을 활용해서 숫자형으로 변경했다. Numpy에서 문자열 데이터는 고정 크기를 가지며 별다른 경고를 출력하지 않고 입력을 임의로 잘라낼 수 있으므로 위 코드처럼 numpy.string_형을 이용할 때는 주의하는 것이 좋다.
